...
位置と速度の更新周期を独立に設定でき、連続・非連続のA/B配線を用途に合わせて選べます。DMAは使いません。

ここでのRP235Xは、RP2350A/B・RP2354A/Bをまとめた呼び方です。
[Raspberry Piの製品情報](https://www.raspberrypi.com/products/rp2350/)と
[検証状況](#検証状況とgithub-actions)も参照してください。
SDKの識別名と試験した型番はRP2350と表記します。すべての品種の実機確認を意味するものではありません。
...
連続方式は上流のPIOプログラムを引き継ぎ、非連続方式は派生した2SMプログラムを使います。
著作者とライセンスの適用範囲は[THIRD_PARTY_NOTICES](THIRD_PARTY_NOTICES.md)を参照してください。
...
**A/BをどのGPIOにつないだか確認し、次のどちらかを自分で選んで書きます。自動切り替えはありません。**
GPIO番号を使います。基板のコネクタの通し番号ではありません。
...
`beginConsecutive(2, 10)` のような離れた指定は初期化エラーになり、別方式へ切り替えません。
...
**初期化結果は必ず確認してください。** 失敗したインスタンスで計測を続けないでください。初期化はピン指定とPIO資源を検査します。エンコーダ信号が実際に届いているかまで自動検出するものではありません。

| 戻り値 | 確認すること |
|---|---|
| 0 | 初期化成功 |
| −1 | 同じ方式・GPIO範囲で使えるPIOが不足。接続台数、他のPIOライブラリ、指定PIOを確認 |
| −2 | ピンの重複・範囲外、連続用への非連続指定、PIO指定、二重初期化を確認 |
| −3 | 初期化時のPIO取得に失敗。資源は返却済み |
...
- Arduinoでは[Earle F. Philhower版arduino-pico](https://github.com/earlephilhower/arduino-pico)を使用します。検証済みコアは6.0.0、実機はRP2040-Zeroです。
- 単独の **Pico SDK 2.1.1** でもビルドとRP2040-Zero実機試験を確認済みです。Arduinoと同じライブラリ本体を使い、Arduinoへの依存はありません。RP2350はビルド確認済み、実機未検証です。[SDK検証記録](docs/PICO_SDK_VALIDATION.md)を参照してください。
- Arduino Mbed RP2040コア、AVR、ESP32には対応しません。
- 別途導入するArduinoライブラリの依存はありません。モーターベンチのWireはボードコアに付属します。
...
3. [Releases](https://github.com/Suzu-Gears/Pico_PIO_Encoder/releases)の `Pico_PIO_Encoder-0.4.0.zip` を「スケッチ → ライブラリをインクルード → .ZIP形式のライブラリをインストール」で追加します。Gitで取得する場合はスケッチブックの `libraries/Pico_PIO_Encoder` に配置します。
...
**RP2350 A2のGPIOエラッタE9:** 入力の漏れ電流により、内部プルダウンだけでは信号がLowへ戻らない場合があります。PIO入力や `Pull::None` でも影響し得ます。3.3V I/Oでの公式対策は、Lowを十分に駆動できる出力、または8.2kΩ以下の外付けプルダウンです。内部プルアップは使えますが、エンコーダ出力がLowを確実に作れる必要があります。**シリコン改版A3・A4では修正済み**です。この改版名はパッケージを表すRP2350A/Bとは別です。[公式データシートのE9と改版履歴](https://datasheets.raspberrypi.com/rp2350/rp2350-datasheet.pdf)を参照してください。ライブラリは指定された抵抗設定を行いますが、この電気的な不具合を解消するものではありません。
...
累積したカウント差を対応するエッジ間の時間で割ります。どちらも同じ計算で、
「低速では周期式、高速ではカウント式へ切り替える」というモード切り替えはありません。
カウントと時間を組み合わせる方式ですが、切替式のハイブリッドとは区別しています。
...
以下は設定の出発点です。機体の制御性能を保証する値ではなく、応答・速度のばらつき・処理時間を実測して調整してください。

| 用途 | 位置取得 | 速度更新 | 設定の考え方 |
|---|---|---|---|
| モーター計測の初期評価 | 1ms（1kHz） | 5～20ms（50～200Hz） | 本ライブラリの実機試験で使用した組み合わせ。まず10msから比較する |
| 速度変化への応答を優先 | アプリケーションの制御周期 | 毎取得（設定値0） | 推定値の公開待ち時間を減らす。制御周期を1msにすれば速度も毎1ms更新 |
...
PIOの生カウンタは32bitです。**64bitの実測位置への積算、速度計算、補間位置、Snapshot公開はCPU側**で行います。そのため「位置のエッジ計数はPIOで自動」ですが、公開される64bit位置までCPUの取得なしに更新されるわけではありません。

取得していない間もPIOは計数を続け、次の取得でカウント差分を回収できます。ただし、カウンタ周回を判別できる最大移動量・最大取得間隔の制約があります。保存済みSnapshotは次の取得まで変わりません。PIOが保持するのは累積カウントと最後の遷移の情報で、すべてのエッジ履歴をCPUへ転送する方式ではありません。
...
位置を読めていても、短い区間の整数差分だけでは速度が0と1000step/sを行き来します。これはCPUが遅いからではなく、エッジが疎らなことによる量子化です。
...
新しいエッジがない状態は、停止している場合だけでなく、低速で次のエッジを待っている場合にも生じます。取得回数を基準に停止を判定すると、取得周期によって停止と見なす時間が変わります。例えば「変化なしが3回」であれば、1ms取得では約3ms、10ms取得では約30msです。
...
`refresh()` は呼び出すたびに取得します。`read()` や位置・速度のgetterは `setMinRefreshIntervalUs()` の取得キャッシュに従います。毎回の `read()` で新しいデータを得たい場合は、このキャッシュ間隔を0にします。
...
速度周期に0以外を指定した場合、期限後の最初の取得で速度を再計算します。バックグラウンドタイマーはなく、取得周期より細かい速度更新は行えません。例えば10msごとの取得に1msの速度周期を指定しても、速度が1msごとに更新されるわけではありません。
...
バックグラウンドタイマーはありません。制御側が取得を呼びます。速度の既定更新間隔は0（毎取得）、自己更新getterの取得キャッシュは100µsです。速度の更新期限が来ていない間は速度値を保持しますが、停止タイムアウトの判定は毎取得行います。

取得に失敗すると直前の値・取得時刻を保持し、`ReadFailed` が立ちます。FIFOタイムアウトでは `PioFault` も立ち、`end()` 後に選んだ方式で再初期化します。`positionValid()` / `speedValid()` は取得失敗も判定します。Snapshotの `read_failures` と `read_retries` が診断用カウンタです。
...
追加の入力は不要です。モーターを原点へ動かす処理ではなく、現在位置に座標を割り当てる操作です。
...
RP2350B系では必要に応じて空きPIOのGPIOウィンドウを選びます。使用中のPIOのウィンドウは変更しません。GPIO15と32のように同じ窓へ収まらない組は拒否します。最後の解放時には元の窓へ戻します。上位GPIOは実機未検証です。使用するチップ・ボードで利用可能なピンを指定してください。
...
- 取得間に往復して同じカウントへ戻る動きは、静止と確実には区別できません。任意の動きで取得周期を変えても推定結果が完全に一致する、という保証はありません。
- 位相校正は、安定した回転中に各stepを観測できる取得頻度が必要です。通常の実測位置の積算とは条件が異なります。速度周期だけを遅くしても校正用の取得頻度は変わりません。周期比較の試験では校正を無効にしてください。
...
RP2040-Zeroで、両配線方式、PIO資源の解放・再利用、位置・速度の独立更新、別コアからのSnapshot取得を確認しています。
Arduinoの検証環境はarduino-pico 6.0.0、単独SDKの実機試験はPico SDK 2.1.1です。
RP2350はビルド確認済みで、Arduino側ではGPIOウィンドウのコードパスも確認しています。
RP2350実機と上位GPIOでの実動作は未検証です。
...
- Pico・Pico 2 ARM向けの連続／非連続SDKサンプルのビルド。

コアとSDKは検証したバージョンに固定しています。CIのコンパイル成功と実機試験は別です。
[配線方式の試験](docs/PIN_MODES_VALIDATION.md)、[SDK試験](docs/PICO_SDK_VALIDATION.md)、
[公開準備時の確認](docs/RELEASE_VALIDATION.md)に結果と範囲を記載しています。
Release作成とLibrary Manager登録の手順は[公開ガイド](docs/RELEASING.md)を参照してください。
このCIは検証を行うもので、Release作成やLibrary Manager登録は自動実行しません。
